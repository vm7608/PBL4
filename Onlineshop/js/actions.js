	...
	*/
	$("#search_btn").click(function(){
		...
		if(keyword != ""){
			...
			url		:	"action.php",
			method	:	"POST",
			data	:	{search:1,keyword:keyword,get_amount:amount,get_sort:sort,pageNumber:1},
			success	:	function(data){ 
				$("#get_product").html(data);
				if($("body").width() < 480){
					$("body").scrollTop(683);
	...
	$("body").delegate("#page","click",function(){
		...
		if (keyword != undefined) {
			...
			if(keyword != ""){
				...
				url		:	"action.php",
				method	:	"POST",
				data	:	{search:1,keyword:keyword,get_amount:amount,get_sort:sort,pageNumber:pn},
				success	:	function(data){ 
					$("#get_product").html(data);
					if($("body").width() < 480){
						$("body").scrollTop(683);
		...
		else if ($(".active").eq(0).hasClass("selectBrand")) {
			$("#get_product").html("<h3>Loading...</h3>");
			var bid = $(".btn.navbar-btn.selectBrand.active").attr('bid');
			var amount = $(".input-select-number").find(":selected").text();
			var sort = $(".input-select-sort").find(":selected").text();
